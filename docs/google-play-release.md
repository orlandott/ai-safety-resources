...
- **Phone screenshots**: at least 2 (up to 8), 9:16 portrait. Five ready-to-upload
  1080×1920 shots are committed in `mobile/assets/play-store/screenshots/`
  (Home, learning path with progress, category list, resource detail, search),
  captured from the app's web build at phone size. Prefer native captures? Run
  `npx expo start --android` in an emulator and retake the same five screens.
