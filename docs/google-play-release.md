...
- **Phone screenshots**: at least 2 (up to 8), 9:16 portrait. Run the app in an
  emulator (`npx expo start --android`) or on device and capture: Home (paths +
  categories), a learning path with progress, a category list, a resource
  detail, and search. No frames or captions needed.
