...
- Accept the declarations, then work through **Dashboard → Set up your app**:
  ...
  accounts, analytics, or tracking; shelves, ratings, and notes stay
    on-device.
...
- **Feature graphic** (1024×500 PNG): `mobile/assets/play-store/feature-graphic.png`
  (both regenerated by `node scripts/make-icons.mjs`, which draws the shield-
  and-book mark used by every icon the app ships)
- **Phone screenshots**: at least 2 (up to 8), 9:16 portrait. Generate them with
  ...
  `mobile/assets/play-store/screenshots/` (Explore with its on-device
  suggestions, a learning path with progress, the library's finished shelf with
  ratings and notes, the Progress tab, a resource detail showing the shelf /
  rating / notes controls, search with the time filter, a category list, plus a
  dark-mode shot).
