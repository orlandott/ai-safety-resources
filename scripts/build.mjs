...
function buildDataExports(enriched, groups, startHere) {
  ...
  const startHereExport = startHere.map((p) => ({
    track: p.track,
    name: p.entry.Name,
    link: p.entry.Link,
    why: p.why,
    ...
    JSON.stringify(
      {
        ...
        generatedFrom: "public/resources.js",
        tracks,
        startHere: startHereExport,
        resources: enriched,
      },
      null,
      2
  ...
  // into its search text without bloating the canonical resources.js, plus the
  // "Start here" pick per track so hydrated cards keep the badge the
  // server-rendered cards carry.
  const tagMap = {};
  for (const e of enriched) {
    if (e.tags.length && e.Name) tagMap[e.Name] = e.tags;
  }
  const startHereMap = {};
  for (const p of startHere) startHereMap[p.track] = p.entry.Name;
  queueWrite(
    ...
    `// Generated by scripts/build.mjs — do not edit by hand.\n` +
      ...
      `window.RESOURCE_START_HERE = ${JSON.stringify(startHereMap)};\n`
  );
}
...
// `startHere` prepends the "Start here" pill on the category's curated pick.
function ssrCard(entry, track, headingLevel = 2, startHere = false) {
...
function ssrPaneMarkup(entries, track, startEntries) {
  if (!entries.length) return "";
  return entries.map((e) => ssrCard(e, track, 4, startEntries.has(e))).join("");
}
...
function injectHomepage(html, groups, startEntries) {
  for (const t of TRACKS) {
    html = injectPane(html, t.pane, ssrPaneMarkup(groups.get(t.key), t.key, startEntries));
  }
...
function categoryPage(track, entries, startEntries) {
  const title = `${track.label} – AI Safety Resources`;
  const desc = track.intro;
  const url = `${SITE_ORIGIN}/${track.slug}/`;
  const cards = entries.map((e) => ssrCard(e, track.key, 2, startEntries.has(e))).join("\n      ");
  const main = [
...
// Resolve the "Start here" picks against the dataset and enforce exactly one
// pick per track, so adding a category without choosing its on-ramp (or
// renaming a picked resource) fails the build instead of silently shipping.
function resolveStartHere(resources, fictionTitles) {
  const picks = START_HERE.map((p) =>
    resolveReference(p, "Start-here pick", resources, fictionTitles)
  );
  const byTrack = new Map(picks.map((p) => [p.track, p]));
  if (byTrack.size !== picks.length) {
    throw new Error("Start-here picks must name at most one resource per track");
  }
  for (const t of TRACKS) {
    if (!byTrack.has(t.key)) {
      throw new Error(`No start-here pick defined for track "${t.key}"`);
    }
  }
  return picks;
}

// Inject the "Start here" on-ramp cards into the homepage marker section.
function injectStartHere(html, picks) {
  const cards = picks
    .map((p) => {
      const label = TRACK_BY_KEY.get(p.track)?.label || "";
      return (
        `<a class="starter-card" href="${escapeHtml(p.entry.Link)}" target="_blank" rel="noopener noreferrer">` +
        `<span class="starter-card-kind">${escapeHtml(label)}</span>` +
        `<span class="starter-card-title">${escapeHtml(p.entry.Name)}</span>` +
        `<span class="starter-card-why">${escapeHtml(p.why)}</span>` +
        `</a>`
      );
    })
    .join("");
  const re = /(<div class="starter-grid" data-build="start-here">)[\s\S]*?(<\/div>)/;
  if (!re.test(html)) {
    throw new Error('Could not find the start-here container (data-build="start-here") in index.html');
  }
...
function main() {
  ...
  const startHere = resolveStartHere(resources, fictionTitles);
  // Compared by object identity: picks resolve to the same entry objects the
  // track groups hold, so no name/link re-matching is needed at render time.
  const startEntries = new Set(startHere.map((p) => p.entry));

  const enriched = enrichResources(resources, fictionTitles);
  buildDataExports(enriched, groups, startHere);

  const resolvedPaths = resolvePaths(resources, fictionTitles);

  let html = fs.readFileSync(indexPath, "utf8");
  html = injectHomepage(html, groups, startEntries);
  html = injectStartHere(html, startHere);
  html = injectPaths(html, resolvedPaths);
  queueWrite(indexPath, html);

  for (const t of TRACKS) {
    queueWrite(
      path.join(publicDir, t.slug, "index.html"),
      categoryPage(t, groups.get(t.key), startEntries)
    );
  }
